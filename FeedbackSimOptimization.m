...
time = linspace(0, 1, 500); %Simulation duration (s)
...
PertPeriod = time(end); %period of perturbation (s) [For only acceleration perturbation torque]
PertAmplitude = -0.1; %Amplitude of perturbation  (Nrad)
PertWidth = 10; %Width of perturbation (% of period)

PertDelay_Acc = 0.1; %Phase delay of perturbation
PertDelay_DeAcc = 0.6; %Phase delay of perturbation

ModelName = 'FeedbackSim';

save('SimVariables','ModelName', 'm', 'L', 'g', 'time', 'ICs', 'tau', 'lambda', 'PertAmplitude', 'PertPeriod', 'PertWidth', 'PertDelay_Acc','PertDelay_DeAcc')
...
Q = [0.0001 0.001 0.01]; %Weighting Vector [Position Velocity Acceleration]
rho = 0.01; %EMG weighting
Omega = 0.1; %Terminal Position weighting

Opt_Gains = fmincon(@(x) RunFBSim_Opt(x, Q, rho, Omega),x0,A,b,Aeq,beq,LB, UB,nonlcon,options_optimization)
